...
3. 持仓实时估值，多数据源切换与对比（天天基金 / 蛋卷 / 自建公开持仓估算）；持仓明细基金名称+代码双显、含「当日收益/估算收益」列、支持按各数据列排序、列顺序拖拽与显隐自定义（本地保存）
4. 自定义买卖策略 + 策略集 + 基于历史数据的区间回测；持仓集合可配置策略集并手动执行（含执行预览，按持仓页估值推算买卖金额，集合间互不影响）
5. 策略集 Base64 导入/导出
6. 个人数据保存在浏览器 localStorage（存储层已抽象，预留微信小程序适配）；多基金行情/信息接口支持顺序/并发调用切换（默认顺序，规避第三方接口 429 限流）
7. 全前端页面移动端访问适配（响应式抽屉导航、表格横向滚动、表单与弹窗自适应）
