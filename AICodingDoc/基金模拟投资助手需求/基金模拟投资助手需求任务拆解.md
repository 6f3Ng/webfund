...
### ADJ-29 回测去掉初始资金限制 + 重算指标 + 基准支持选策略/基金 + 策略与持仓集合批量管理/复制

> 本次一并交付五项用户需求：(1) 回测算法去掉初始资金限制并重算各项指标；(2) 单策略回测「基准标的」支持选一条策略 / 输入基金代码 / 默认首个标的；(3) 多策略回测「基准标的」同步改造，并修复非首个基准不生效问题；(4) 策略列表批量删除 + 批量修改标的基金代码；(5) 策略集与持仓集合复制副本 + 批量删除。

#### 需求 1：回测去掉初始资金限制 + 重算指标（R9 增强）

- **背景/问题**：原回测引擎 BUY 分支 `if (amount <= 0 || amount > cash) continue`——当策略所需买入金额超过可用现金时直接跳过，导致初始资金不足时策略无法完整执行，收益口径失真。用户希望去掉该限制，并以更贴合"真金白银投入"的口径重算各项关键指标。
- **变更（`@fund/core`）**：
  - **去掉买入现金上限**：BUY 分支改为 `if (amount <= 0) continue`，现金可为负（代表累计净投入超过期初资金，即"追加投入"），完整反映策略真实资金需求。
  - **策略求值不受现金限制**：逐日循环中 `buildContext` 传入 `Infinity` 作为评估现金（各 evaluator 的 `ctx.cash < amount` 守卫不再阻断触发），真实现金 `cash` 仍单独累计。注意：实盘求值 `live.ts` 不受影响，仍按真实可用现金限制下单。
  - **重算/新增指标 `BacktestMetrics`**：
    - `netInvested`（实际投入成本 = 累计买入 − 累计卖出回收）——作为**累计收益率**的基准；
    - 新增 `finalHoldingShares`（期末持有份额 = 所有持仓份额之和）；
    - 新增 `finalCostPrice`（期末成本单价 = 期末持仓成本 / 期末持有份额）；
    - 新增 `finalUnitNav`（期末实际单价 = 期末持有总额 / 期末持有份额，持仓加权市价）；
    - 新增 `cumulativeReturn`（累计收益率 = 期末总收益 / 实际投入成本，以真金白银投入为基准，不受闲置现金影响）；
    - 新增 `holdingProfitRate`（期末持有收益率 = 持仓浮盈 / 持仓成本，金额口径）；
    - 保留 `finalHoldingValue`（期末持有总额）、`holdingProfit`（期末持有收益）、`totalReturn`（旧口径总收益率，受闲置现金影响）、`holdingReturn`（时间加权持有收益率）等既有字段，向后兼容既有测试与对比表。
- **变更（web，`BacktestPage`）**：回测指标卡片
  - 「资金」组：「累计净投入」改名为「实际投入成本」（附 Tooltip 说明为累计收益率基准）；
  - 「期末状态」组：新增「期末持有份额 / 期末成本单价 / 期末实际单价」，「期末持有资产」改名「期末持有总额」，「期末可用现金」附 Tooltip 说明可为负；
  - 「收益」组：新增「累计收益率」（净投入口径）与「期末持有收益率」（金额口径），「持仓浮盈」改名「期末持有收益」，「总收益率」「持有收益率(时间加权)」附 Tooltip 区分口径。
- **测试**：core 新增 3 个用例（买入超可用现金仍成交且现金为负 / 期末份额·成本单价·实际单价·累计收益率·持有收益率口径正确 / 基准三态）。浏览器端到端实测真实 161725+110011（2021，每月各 ¥1000）：实际投入成本 ¥24,000、期末持有份额 10,152.40、期末成本单价 2.3640（=24000/10152.40）、期末实际单价 2.1629（=21958.88/10152.40）、累计收益率 −8.50%（=−2041.12/24000）、期末持有收益 −2,041.12 / 收益率 −8.50%，全部口径自洽。

##### 需求 1 后续：去掉「初始资金」输入字段（initialCash 改为可选 + 自动推导）

- **背景**：去掉买入现金上限后，「初始资金」字段已不影响成交轨迹（仅作现金平移基准），但页面仍强制要求填写，体验冗余。用户要求进一步去掉该字段。
- **变更（`@fund/core`）**：`BacktestInput.initialCash` 改为**可选**。回测内部以 0 起点累计现金流并记录最低点 `minCash`，循环结束后确定期初资金：
  - 提供 `initialCash` → 直接采用（现金可为负，代表追加投入，行为不变）；
  - **未提供** → 自动取现金缺口最大值 `max(0, −minCash)`，整体平移现金/总资产曲线，使期间可用现金恰好不为负，即反映「按策略买卖到底需要准备多少钱」。`metrics.initialCash`、`totalReturn`、`annualizedReturn` 及基准均改用该推导值。
- **变更（web）**：`BacktestPage`（单策略）与 `ComparisonPanel`（多策略对比）均移除「初始资金」`InputNumber` 表单项及相关 state，不再向引擎传 `initialCash`。单策略页期初资金改取回测结果（用于曲线「期初资金」参考线），指标卡「期初可用资金」改名「期初所需资金」并附 Tooltip 说明为自动推导；对比页图表基线取各结果 `initialCash` 的最大值。
- **测试**：core 新增「不提供 initialCash 自动推导、期间现金不为负」用例（core 206→207）；`pnpm -r typecheck`、`pnpm -r test`（core 207 + web 32 + workers 40 = **279 全绿**）、core+web 生产构建均通过。浏览器实测真实 161725（2021 每月 ¥2000，纯定投）：回测表单已无「初始资金」项；运行后「期初所需资金 ¥24,000」（=累计买入）、期末可用现金 ¥0.00（全程不为负）、累计收益率 = 总收益率 −5.48%（无闲置现金，两口径重合）、期末持有份额/成本单价/实际单价等全部自洽；控制台仅既有 antd Form 开发期告警。

#### 需求 2：单策略回测「基准标的」支持选策略 / 基金 / 默认（R9 增强）

- **需求**：单策略回测的「基准标的」支持选择一条策略集作为对比；未选则支持输入基金代码作参考；为空则保持原「默认首个标的」买入持有。
- **变更（`@fund/core`）**：
  - `BacktestInput` 新增 `benchmarkStrategies?` / `benchmarkConflictPolicy?` / `benchmarkLabel?`；基准优先级 **benchmarkStrategies（按策略回测）> benchmarkFundCode（指定基金买入持有）> 首个标的买入持有**。
  - `BenchmarkResult` 新增 `kind: 'BUY_HOLD' | 'STRATEGY'`、`fundCode?`（改为可选）、`label?`。新增 `buildBenchmarkResult`：策略基准以相同区间/资金/费率独立 `runBacktest`（不再向下传 benchmark，避免递归）后包装为 `STRATEGY` 基准，曲线取该子回测的总资产曲线。
- **变更（web，`BacktestPage`）**：原单一「基准标的」下拉拆为「基准策略集」下拉（排除当前策略集、仅含非空策略集）+「基准基金」输入框（6 位代码，未选基准策略集时生效，留空则默认首个标的）。`handleRun` 据所选基准把基准策略集/基金标的并入 `loadNavData` 的拉取范围，确保基准所需净值齐备；基准 Alert 与净值曲线图例随 `kind` 标注「策略：xxx」或「xxx 买入持有」。
- **测试**：core 新增基准用例（指定基金买入持有 / benchmarkStrategies 优先生效）。浏览器实测：基准基金 000001 → Alert「基准（000001 买入持有）：总收益 −12.74%… 超额 +10.70%」；基准策略集「白酒定投 (副本)」→ Alert「基准（策略：白酒定投 (副本)）… 超额 +0.00%」（两集相同，超额 0 正确）。

#### 需求 3：多策略回测「基准标的」同改造 + 修复非首个基准不生效（R9 修正）

- **问题**：`ComparisonPanel` 原 `subNav` 仅取各策略集自身标的净值，若所选基准基金不在该集标的内（如非首个标的或外部基金），其净值缺失 → 基准曲线为空、基准不生效。
- **变更（web，`ComparisonPanel`）**：基准字段与单策略一致（「基准策略集」+「基准基金」）；`handleRun` 把基准策略集/基金标的并入一次性 `loadNavData` 拉取，并在每个策略集的 `subNav` 子集中**补入基准标的净值**（修复非首个/外部基准基金无净值导致基准失效）；各策略集统一传 `benchmarkStrategies` 或 `benchmarkFundCode`。`ComparisonChart` 基准曲线名称随 `kind` 标注。
- **测试**：浏览器实测两策略集对比、基准基金 000001（不在任一策略集标的内，修复前会失效）→ 对比表与「总资产曲线对比」图正常渲染、基准曲线存在，控制台 0 错误。

#### 需求 4：策略列表批量删除 + 批量修改标的基金代码（R7 增强）

- **变更（web）**：
  - `strategyStore` 新增 `removeStrategies(setId, ids)`（批量删除）与 `updateStrategiesFundCode(setId, ids, fundCode)`（批量改标的）。
  - `StrategiesPage` 策略列表表格加 `rowSelection`；选中后显示批量工具条——「批量修改标的」（弹窗输入 6 位代码统一改）与「批量删除」（Popconfirm 确认）+「取消选择」；切换策略集时清空选择。
- **测试**：浏览器实测——选中「成长月定投（000001）」→ 批量修改标的为 110011（列即显示「易方达优质精选混合(QDII) 110011」）；批量删除选中策略生效。

#### 需求 5：策略集 / 持仓集合复制副本 + 批量删除（R2/R8 增强）

- **变更（web）**：
  - `strategyStore` 新增 `duplicateSet(id)`（经 `exportStrategySet → importStrategySet` 往返实现深拷贝 + 新 id + 重名「(副本)」）与 `removeSets(ids)`（批量删除，currentId 失效时回退）。`StrategiesPage` 工具栏加「复制副本」按钮；新增「批量管理策略集」区（多选 + 批量删除 Popconfirm）。
  - `portfolioStore` 新增 `duplicate(id)`（经 `exportPortfolio → importPortfolio` 往返深拷贝 + 新 id + 副本名，异步预取名称）与 `removeMany(ids)`。`PortfoliosPage` 行操作加「复制」按钮；表格加 `rowSelection` + 批量删除工具条（Popconfirm）。
- **测试**：浏览器实测——策略集「复制副本」生成「白酒定投 (副本)（2）」（策略完整复制、切换为当前）；多选副本「批量删除（1）」确认后移除、currentId 正确回退。持仓集合「复制」生成「测试组合 (副本)」；全选 2 个「批量删除」确认后清空。

#### 实现范围与验证
- **范围**：需求 1/2 改动 `@fund/core`（backtest 引擎/类型 + metrics）与 web；需求 3/4/5 主要位于 `packages/web`（store + 页面/组件），复用既有序列化 codec 实现复制。未改动 `packages/workers`。
- **测试与构建**：core 新增 6 个 backtest 用例（core 201→207）；`pnpm -r typecheck`、`pnpm -r test`（core 207 + web 32 + workers 40 = **279 用例全绿**）、core+web 生产构建全部通过；修改文件 eslint 0 错误。
- **端到端**：五项需求均经浏览器实测通过，控制台无本次改动引入的运行时错误（仅 antd Form 既有「circular references」开发期告警）；实测后已清理 localStorage 测试数据与临时文件。

> 累计测试：core 207 + web 32 + workers 40 = 279 用例全绿；core/web/workers 全量 typecheck 通过，core+web 生产构建通过。

### ADJ-30 最大回撤负值展示 + 修复"未修复回撤"语义 + 新增回撤修复天数（回测 / 选基）

- **现象（用户反馈）**：回测与选基页的最大回撤展示为**正值**（如 `+30.52%`），看起来像上涨；回撤代表下跌应以**负值**展示；且持续下跌、期末回撤未修复时展示有误；此外希望新增"最大回撤修复天数"。
- **根因/澄清**：
  - 回撤**幅度**计算本身正确（峰值→谷底跌幅，正数存储）；问题在**展示**——用 `fmtPct` 渲染为 `+x%`（带正号、红色），与"下跌"语义相反。
  - "未修复"此前无任何展示，用户无法判断回撤是否已修复及持续多久。
- **变更（`@fund/core`）**：
  - `drawdownDetail` 扩展：在原峰值/谷底基础上，新增**修复探测**——谷底之后净值/资产首次回到该回撤峰值的日期 `recoveryDate` 与修复天数 `recoveryDays`（谷底→修复的交易日数）；期末仍未修复时返回 `daysSinceTrough`（谷底至序列末的交易日数），`recoveryDate/recoveryDays` 为 `undefined`。
  - `BacktestMetrics` 新增 `maxDrawdownRecoveryDate / maxDrawdownRecoveryDays / maxDrawdownDaysSinceTrough`（基于持有指数曲线，与持有最大回撤同源）。
- **变更（web）**：
  - `utils/format.ts` 新增 `fmtDrawdown(v)`（正幅度按负值展示，如 `0.3052 → "-30.52%"`，0 显示 `"0.00%"`）与 `drawdownColor(v)`（有回撤按下跌绿色）。
  - `BacktestPage`：「总资产最大回撤」「持有最大回撤」改用 `fmtDrawdown`/`drawdownColor`（负值+绿色）；基准 Alert 的「最大回撤」同改；新增「回撤修复天数」指标卡——已修复显示 `N 天`（Tooltip 注明谷底→修复日），未修复显示 `未修复（N天）`（红色，Tooltip 注明已持续天数），无回撤显示 `—`。
  - `ComparisonPanel`：指标对比表「持有最大回撤」行改用 `fmtDrawdown`。
  - `FundPickerPage` + `fundPickerService`：`FundMetrics` 新增同样的修复字段并由 `computeFundMetrics` 填充；指标对比表「最大回撤」行改用 `fmtDrawdown`；每只基金详情卡「最大回撤」改负值展示并新增「回撤修复天数」统计（语义同回测）。
- **测试**：core 新增 2 个 `drawdownDetail` 用例（谷底后回到峰值的修复天数 / 期末未修复返回 daysSinceTrough），core 207→209。`pnpm -r typecheck`、`pnpm -r test`（core 209 + web 32 + workers 40 = **281 用例全绿**）、core+web 生产构建均通过。
- **端到端（本地 `wrangler dev` + 浏览器实测真实 161725）**：
  - 选基 2021 全年：最大回撤 **-30.52%**（负值绿色）、回撤修复天数 **59 天**；与接口数据核对——峰值 1.6198(02-10)→谷底 1.1255(03-09)→2021-06-04 回到 ≥1.6198，恰为谷底后 59 个交易日，吻合。
  - 选基 2021-02-10~03-09（结束即谷底）：最大回撤 **-30.52%**、回撤修复天数 **"未修复（0天）"**——正是"持续下跌、期末未修复"场景。
  - 回测（161725 底仓，2021-02-10~08-20）：总资产/持有最大回撤均 **-30.52%**、回撤修复天数 **59 天**。控制台仅既有 antd Form 开发期告警。
- **实现范围**：`@fund/core`（`metrics.drawdownDetail` + `BacktestMetrics`）与 `packages/web`（format 工具 + 回测/选基/对比展示）；未改动 `packages/workers`。实测后已清理 localStorage 与临时文件。

> 累计测试：core 209 + web 32 + workers 40 = 281 用例全绿；core/web/workers 全量 typecheck 通过，core+web 生产构建通过。

### ADJ-31 当前最大回撤未修复时补充展示"历史已修复最大回撤"（回测 / 选基）

- **需求（用户反馈）**：在 ADJ-30 基础上，当**当前最大回撤未修复**时，额外展示**历史上已修复的最大回撤**的幅度与修复天数，便于判断"这只标的/策略历史上最深扛过多大回撤、用了多久修复"。
- **变更（`@fund/core`）**：
  - `drawdownDetail` 增加"历史已修复最深回撤"统计：遍历曲线，每当净值**回到/超过上一峰值**（`value >= peak`）即视为上一峰值对应的回撤区间已修复，结算该区间的最深回撤；取所有已修复区间中最深者，输出 `recoveredMaxDrawdown`（正数幅度）、`recoveredTroughDate`、`recoveredRecoveryDate`、`recoveredRecoveryDays`（谷底→修复交易日数）。无已修复区间则为 0。
  - `BacktestMetrics` 新增 `recoveredMaxDrawdown / recoveredMaxDrawdownTroughDate / recoveredMaxDrawdownRecoveryDate / recoveredMaxDrawdownRecoveryDays`（基于持有指数曲线，与持有最大回撤同源）。
- **变更（web）**：
  - `BacktestPage` 风险区与 `FundPickerPage` 详情卡：**仅当**当前最大回撤未修复（`maxDrawdownRecoveryDays === undefined` 且回撤>0）**且**存在历史已修复回撤（`recoveredMaxDrawdown > 0`）时，新增「历史已修复最大回撤」统计，显示 `-XX.XX% / N天`（负值绿色），Tooltip 注明谷底→修复日期与天数。已修复（当前回撤本身已修复）时不展示该项，避免冗余。
  - `fundPickerService` `FundMetrics` 同步新增字段并由 `computeFundMetrics` 填充。
- **测试**：core 新增 2 个 `drawdownDetail` 用例（当前回撤未修复时给出历史已修复最大回撤 / 全程已修复时历史已修复回撤即最大回撤），core 209→211。`pnpm -r typecheck`、`pnpm -r test`（core 211 + web 32 + workers 40 = **283 用例全绿**）、core+web 生产构建均通过。
- **端到端（本地 `wrangler dev` + 浏览器实测真实 161725，选基 2021-01-01~2022-04-30）**：当前最大回撤 **-40.19%**（峰值 2021-06-07→谷底 2022-04-11，**未修复（14天）**）；新增「历史已修复最大回撤 **-30.52% / 59天**」（即 2021 年初 Feb→Mar 的回撤，已于 2021-06-04 修复，与接口数据核对吻合）。当前回撤已修复的区间（如 2021 全年）则不展示该项。控制台仅既有 antd Form 开发期告警。
- **实现范围**：`@fund/core`（`metrics.drawdownDetail` + `BacktestMetrics`）与 `packages/web`（回测/选基展示 + `fundPickerService`）；未改动 `packages/workers`。实测后已清理 localStorage 与临时文件。

> 累计测试：core 211 + web 32 + workers 40 = 283 用例全绿；core/web/workers 全量 typecheck 通过，core+web 生产构建通过。

#### ADJ-31 后续：选基「区间业绩指标对比」表加入"回撤修复天数"与"历史已修复最大回撤"

- **需求**：选基的「历史已修复最大回撤」与「回撤修复天数」此前仅在每只基金的详情卡展示，需同时进入「区间业绩指标对比」表格，便于多基金横向对比。
- **变更（web，`FundPickerPage`）**：`MetricRow` 新增可选 `renderCell(f): ReactNode` 支持非纯数值的自定义单元格；指标对比表在「最大回撤」行后插入两行——
  - 「回撤修复天数」：已修复显示 `N 天`、未修复显示 `未修复（N天）`、无回撤显示 `—`；
  - 「历史已修复最大回撤」：仅当该基金当前回撤未修复且存在历史已修复回撤时显示 `-XX.XX% / N天`（负值绿色），否则 `—`。
  两行 `better:'none'` 不参与 ★ 最优高亮（描述性指标）。表格列渲染优先调用 `renderCell`，回退原 `fmt`。
- **测试**：web typecheck + 全量测试通过（32 用例）、web 生产构建通过。浏览器实测真实 161725 + 000001（2021-01-01~2022-04-30）对比表两行正确呈现：回撤修复天数「未修复（14天）/未修复（3天）」、历史已修复最大回撤「-30.52% / 59天」「-5.09% / 7天」，与各自详情卡一致；控制台仅既有 antd Form 开发期告警，实测后已清理。

#### ADJ-31 后续 2：所有"最大回撤"数值统一用绿色展示

- **需求**：将所有最大回撤参数的数值用绿色展示（A股习惯：下跌为绿）。
- **变更（web）**：`utils/format.ts` 的 `drawdownColor` 改为**恒返回绿色** `#3f8600`（原先 0 回撤返回默认色）；回测页「总资产/持有最大回撤」「历史已修复最大回撤」、选基详情卡「最大回撤/历史已修复最大回撤」本就走 `drawdownColor` 自动变绿；选基「区间业绩指标对比」表为「最大回撤」行新增 `drawdown` 标记并按 `drawdownColor` 着色，回测多策略对比表「持有最大回撤」行同改。回撤修复天数的「未修复」仍保留红色警示（非回撤幅度值，表示风险未解除）。
- **测试**：web typecheck + 测试（32）+ 生产构建通过。浏览器实测：回测页总资产/持有最大回撤 `-40.19%`、历史已修复 `-30.52% / 59天` 均为绿色 `rgb(63,134,0)`；选基详情卡与对比表「最大回撤」（含 ★ 最优者）均为绿色。实测后已清理。

### ADJ-32 多策略对比指标补全 + 策略集/持仓集合合并 + 解除基准策略同集合限制

> 本次一并交付三项用户需求：(1) 多策略集对比的指标对比表补全为「单策略集回测」可见的全部指标；(2) 新增策略集合并与持仓集合合并功能（多个集合并为一个）；(3) 单策略回测的「基准策略集」去掉"不能与回测策略集相同"的限制，回测策略与基准策略可选同一个。

#### 需求 1：多策略集对比指标对比表补全（R9 增强）

- **背景/问题**：「多策略集对比」的指标对比表仅展示 13 项指标（期末总资产 / 总收益率 / 年化 / 持有收益率 / 持有最大回撤 / 年化波动率 / 夏普 / 索提诺 / 卡玛 / 盈利日占比 / 累计净投入 / 累计费用 / 交易次数），而「单策略集回测」展示 30+ 项分组指标，对比时缺少期初资金、期末状态明细、累计收益率、回撤修复天数等多项数据。
- **变更（web，`ComparisonPanel`）**：`METRIC_ROWS` 扩充为与单策略回测**完全一致**的全部指标，并按相同分组顺序排列——
  - **资金**：期初所需资金 / 累计买入 / 累计卖出回收 / 实际投入成本；
  - **期末状态**：期末总资产 / 期末可用现金 / 期末持有总额 / 期末持有份额 / 期末持仓成本 / 期末成本单价 / 期末实际单价；
  - **收益**：期末总收益 / 累计收益率 / 总收益率 / 年化收益 / 期末持有收益 / 期末持有收益率 / 持有收益率(时间加权)；
  - **风险**：总资产最大回撤 / 持有最大回撤 / 回撤修复天数 / 历史已修复最大回撤 / 年化波动率 / 盈利日占比；
  - **风险调整收益**：夏普比率 / 索提诺比率 / 卡玛比率 / 持有年化收益；
  - **交易**：交易次数（买/卖）/ 累计费用 / 回测交易日。
  - `MetricRow` 新增可选 `color`（`'pnl'` 红涨绿跌 / `'drawdown'` 回撤绿）与 `renderCell`（非纯数值指标自定义单元格：回撤修复天数「N 天 / 未修复（N天）/ —」、历史已修复最大回撤「-XX.XX% / N天」仅当前回撤未修复时显示、交易次数「N（买x/卖y）」）。列渲染优先 `renderCell`，`better!=='none'` 的数值指标仍按越大/越小用 ★ 高亮最优、收益类红绿着色、回撤绿色。
- **实现范围**：完全位于 `packages/web`，复用既有 `BacktestMetrics` 全部字段与 `utils/format` 工具，未改动 `@fund/core`。

#### 需求 2：策略集合并 + 持仓集合合并（R2/R8 增强）

- **策略集合并（`strategyStore.mergeSets(ids, name)`）**：新建空策略集（`createStrategySet`），将所选 2+ 策略集的全部策略并入，**重新分配策略 id**（`generateId('st')`）避免跨集合 id 冲突；冲突归并策略沿用第一个来源集合。来源集合保留不变。`StrategiesPage`「批量管理策略集」区新增「合并为新策略集（N）」按钮（选中 ≥2 时可用）+ 合并弹窗（名称输入，重名校验）。
- **持仓集合合并（`portfolioStore.merge(ids, name)`）**：复用 `createPortfolio` 工厂以与新建/编辑一致口径重建——可用现金相加（`Σ pf.cash`）、同基金持仓按份额合并、成本加权平均（纯函数 `mergePortfolioPositions`：份额相加、成本相加、`costPrice = 合并成本/合并份额`、买入日期取最早 lot），收益基准 = 合并后现金 + 持仓成本之和。**在途守卫**：合并会清空流水/在途（工厂重建语义同编辑），故任一来源存在在途交易（`hasInFlightState`）时拒绝合并并提示。`PortfoliosPage` 批量工具条新增「合并为新集合（N）」按钮（选中 ≥2 且无在途时可用，有在途时禁用并 Tooltip 说明）+ 合并弹窗。来源集合保留不变。
- **实现范围**：完全位于 `packages/web`（store + 页面），复用 `@fund/core` 既有工厂与 `generateId`，未改动 `@fund/core`。

#### 需求 3：基准策略集去掉"不能与回测策略集相同"限制（R9 修正）

- **现象（用户反馈）**：单策略回测第一次回测后不刷新页面直接进行第二次不同策略的回测时，基准策略集无法选择第一次回测的策略；本质是基准策略集下拉用 `s.id !== form.getFieldValue('setId')` 过滤掉了与当前回测策略集相同的项，导致"回测策略与基准策略不能选同一个"。
- **变更（web，`BacktestPage` `SingleBacktest`）**：移除基准策略集下拉的 `s.id !== setId` 过滤（仅保留"非空策略集"过滤）；`handleRun` 中基准解析去掉 `v.benchmarkSetId !== set.id` 限制，仅以"是否选了基准策略集"判定。回测策略与基准策略现可选同一策略集（此时基准为该策略自身回测，超额收益恒为 0，符合预期）。
- **实现范围**：完全位于 `packages/web`，未改动 `@fund/core`。

#### 实现范围与验证

- **范围**：三项需求均完全位于 `packages/web`，未改动 `@fund/core`/`packages/workers`。
- **测试**：web 新增 `stores/portfolioStore.test.ts`（`mergePortfolioPositions`：同基金份额合并/成本加权/取最早日期、不同基金保留/份额为 0 忽略、空集合，共 3 用例，web 32→35）；`pnpm -r typecheck`、`pnpm -r test`（core 211 + web 35 + workers 40 = **286 用例全绿**）、core+web 生产构建均通过；修改文件 eslint 0 错误。
- **端到端（本地 `pnpm dev` + 浏览器实测真实 161725 / 000001 / 110011）**：
  - **持仓集合合并**：稳健组合（现金 10 万 + 161725 1000份@1.30）+ 激进组合（现金 5 万 + 161725 500份@1.50 + 000001 2000份@1.00）→ 合并组合「初始资金 154,050.05 / 可用现金 150,000.00 / 持仓数 2」；现金 = 100000+50000=150000，161725 合并 1500 份（成本 1300+750=2050，单价 1.3667）、000001 2000 份，基准 = 150000 + 2050.05 + 2000 = 154,050.05 正确；来源集合保留。
  - **策略集合并**：白酒定投（161725 月定投×1）+ 成长定投（000001、110011 月定投×2）→「白酒+成长合并（3）」含 3 条策略、标的 [000001,110011,161725]、策略 id 全部重新分配（不与来源 id 冲突）；来源集合保留。
  - **基准同集合**：单策略回测策略集与基准策略集均选「白酒+成长合并」→ 基准下拉正常列出该集合、运行成功，基准 Alert 显示「基准（策略：白酒+成长合并）… 策略相对基准超额 +0.00%」（策略=基准，超额 0 正确）。
  - **对比指标补全**：白酒定投 vs 成长定投（2024 全年）对比表呈现全部 31 项指标（资金/期末状态/收益/风险/风险调整/交易六组），★ 高亮最优、收益类红绿、回撤负值绿色、回撤修复天数「未修复（69天）/157 天」、历史已修复最大回撤「-11.39% / 21天 / —」、交易次数「12（买12/卖0）」均正确渲染。
  - 控制台仅既有 antd Form「circular references」开发期告警，无本次改动引入的运行时错误；实测后已清理 localStorage 测试数据。

> 累计测试：core 211 + web 35 + workers 40 = 286 用例全绿；core/web/workers 全量 typecheck 通过，core+web 生产构建通过。

### ADJ-33 阈值卖出 / 智能阈值卖出支持「按金额 / 按份额 / 按仓位」三种卖出方式（R7 增强）

- **需求**：对「阈值卖出（涨幅触发）」与「智能阈值卖出·涨跌幅」两种策略，调整卖出方式——默认仍按金额卖出（兼容当前导入的旧策略），并支持切换为按份额卖出与按仓位（比例）卖出。
- **设计要点**：
  - `StrategyAction` 早已同时支持 `amount`（金额，按净值换算份额）/ `shares`（绝对份额）/ `ratio`（相对持有份额比例）三种卖出表达，回测引擎与实盘执行器（`portfolioStore.executeActions`）均按既有优先级 `ratio > amount > shares` 落地。本次仅在两个卖出策略的参数与求值层"选择产出哪一种字段"，无需改动引擎/执行/冲突归并/序列化校验。
  - **向后兼容**：新增字段全部可选，`sellMode` 缺省即视为 `AMOUNT`，旧导入数据（仅含 `amount`/`baseAmount`、无 `sellMode`）行为完全不变；序列化 `validateStrategy` 对 `params` 原样透传，无需迁移。
- **变更（`@fund/core`）**：
  - `domain/strategy.ts`：新增 `export type ThresholdSellMode = 'AMOUNT' | 'SHARES' | 'RATIO'`；
    - `ThresholdSellParams` 增加可选 `sellMode`、`sellShares`（份额）、`sellRatio`（仓位比例 0~1），保留 `amount` 作金额模式值（兼容旧数据）；
    - `SmartThresholdSellChangeParams` 增加可选 `sellMode`、`baseShares`、`baseRatio`，保留 `baseAmount`。
  - `strategy/evaluators.ts`：抽出 `resolveSellQuantity(mode, amount, shares, ratio, scale)` 纯函数——按 `sellMode` 产出 `{ amount }`/`{ shares }`/`{ ratio }` 字段与展示文案，量为非正返回 undefined；`SHARES` 经 `roundShares` 规整、`RATIO` 经 `clamp(…,0,1)` 封顶 100%。`evalThresholdSell` 与 `evalSmartThresholdSellChange` 改为调用该函数（智能模式传入随涨幅放大的 `factor` 作 `scale`），触发原因文案随方式显示「¥金额 / N份 / N%仓位」。
- **变更（`packages/web`）**：
  - `StrategyModal.tsx`：新增复用子组件 `SellModeFields`（两策略共用，`smart` 标记区分基准量字段名），含「卖出方式」下拉（按金额/按份额/按仓位）+ 随方式切换的量输入（金额元 / 份额份 / 仓位 %）。三个量字段（`amount/sellShares/sellRatioPct` 与 `baseAmount/baseShares/baseRatioPct`）始终注册，由 `buildParams` 按 `sellMode` 取用、`paramsToForm` 回填（缺省 `AMOUNT`）、`defaultFormForTemplate` 提供默认（`AMOUNT` + 各模式默认值），保证切换方式时各值保留、编辑回填正确；仓位比例按既有惯例以 % 录入、存储为 0~1。
  - `StrategiesPage.tsx`：新增 `describeSellQty(sellMode, amount, shares, ratio)`，列表参数描述按方式显示「¥金额 / N份 / N%仓位」（缺省金额）。
  - 实盘预览摘要 `describeAction` 早已覆盖 ratio/amount/shares，无需改动。
- **测试**：core 新增 5 个用例（`evaluators.test.ts`「卖出方式」：THRESHOLD_SELL 缺省按金额/按份额/按仓位、SMART 按份额随涨幅放大、SMART 按仓位放大并封顶 100%），core 211→216。`pnpm -r typecheck`、`pnpm -r test`（core 216 + web 35 + workers 40 = **291 用例全绿**）、core+web 生产构建均通过；修改文件 eslint 0 错误。
- **端到端（本地 `pnpm dev` + 浏览器实测真实 161725）**：
  - **向后兼容**：注入旧格式策略（无 `sellMode`）→ 列表显示「卖 ¥2000」「基准卖¥1000」（金额模式）；编辑弹窗「卖出方式」默认「按金额（元）」、字段为「卖出金额（元）」并回填原值。
  - **切换份额**：阈值卖出改为「按份额（份）」→ 字段变「卖出份额（份）」，存 500 → `params={sellMode:'SHARES', amount:0, sellShares:500}`、列表显示「卖 500份」。
  - **切换仓位**：智能阈值卖出改为「按仓位（%）」→ 字段变「基准卖出仓位比例（%）」，存 20% → `params={sellMode:'RATIO', baseAmount:0, baseRatio:0.2}`、列表显示「基准卖20%仓位」。
  - **执行验证**：底仓¥100000 + 阈值卖出（按份额 2000 份）回测 2024 全年 → 每次触发卖出恰为 **2000.00 份**（原因「近5日涨X%触发卖出2000.00份」、金额=2000×当日净值），份额方式端到端落地正确。
  - 控制台仅既有 antd Form「circular references」开发期告警，无本次改动引入的运行时错误；实测后已清理 localStorage 测试数据与临时文件。
- **实现范围**：`@fund/core`（类型 + 求值器）与 `packages/web`（策略表单 + 列表描述）；未改动 `packages/workers`、回测引擎、实盘执行器、序列化校验与冲突归并（均已支持三种卖出表达）。

> 累计测试：core 216 + web 35 + workers 40 = 291 用例全绿；core/web/workers 全量 typecheck 通过，core+web 生产构建通过。

### ADJ-34 单策略回测操作流水表新增「持有总份额」「持有总金额」两列（R9 增强）

- **需求**：单策略集回测的「操作流水」表格增加两列——持有总份额、持有总金额。
- **设计**：回测引擎逐笔成交时已维护各基金持仓（份额/成本），在每笔成交记录中额外快照「成交后该基金的持有总份额」与「持有总金额（= 持有总份额 × 当日成交净值）」。卖出后份额相应减少（减至 0 时两值为 0）；同一交易日多策略对同标的的买卖经冲突归并为一笔，份额累计正确。
- **变更（`@fund/core`）**：
  - `strategy/backtest-types.ts`：`BacktestTrade` 新增 `holdingShares`（成交后持有总份额）与 `holdingValue`（成交后持有总金额）。
  - `strategy/backtest.ts`：BUY/SELL 两分支在 `trades.push` 时写入 `holdingShares = pos.shares`、`holdingValue = roundAmount(pos.shares × nav)`（SELL 在份额扣减后、删除空持仓前取值，保证减至 0 也正确）。
- **变更（`packages/web`）**：`pages/BacktestPage.tsx` 的 `tradeColumns` 在「份额」与「原因」之间插入「持有总份额」（保留 2 位）与「持有总金额」（`fmtMoney`）两列。
- **测试**：core 新增 2 个用例（`backtest.test.ts`：跨月定投后持有总份额/总金额累计正确、按份额卖出后两值相应减少），core 216→218。`pnpm -r typecheck`、`pnpm -r test`（core 218 + web 35 + workers 40 = **293 用例全绿**）、core+web 生产构建均通过；修改文件 eslint 0 错误。
- **端到端（本地 `pnpm dev` + 浏览器实测真实 161725）**：底仓¥50000 + 月定投¥5000 回测 2024-01~06 → 操作流水表头为 日期/方向/基金/净值/金额/份额/**持有总份额**/**持有总金额**/原因；逐行核对——首笔买入 ¥55000（净值 0.9201）后持有总份额 58892.72、持有总金额 54,187.19（=58892.72×0.9201），次月定投后累计 64843.56 份、金额 53,677.50（按当日净值 0.8278 计），各行份额累加与金额=份额×当日净值均吻合。控制台仅既有 antd Form 开发期告警，实测后已清理 localStorage 测试数据与临时文件。
- **实现范围**：`@fund/core`（`BacktestTrade` 类型 + 引擎成交记录）与 `packages/web`（回测流水表列）；未改动 `packages/workers`、冲突归并与实盘执行。

> 累计测试：core 218 + web 35 + workers 40 = 293 用例全绿；core/web/workers 全量 typecheck 通过，core+web 生产构建通过。

### ADJ-35 新增赎回费（卖出手续费），A/C 类分开配置（R1/R9 增强）

- **需求**：增加卖出手续费（赎回费），A 类与 C 类分开设置——默认 A 类 0.5%、C 类 0.5%，支持用户手动配置；配置方式与计算方式与既有「申购费」（ADJ-22）一致。
- **设计**：完全镜像 ADJ-22 的 A/C 申购费机制——
  - 设置项 A/C 两个费率 + 默认值；`settingsStore` 派生同步到 `fundInfoService`；交易结算时由 `fundInfoProvider` 按「份额类别（`detectShareClass` 识别 A/C）+ 当前设置」解析费率；回测按统一 A 费率（与申购费在回测中一致，回测不区分 per-fund 份额类别）。
  - 赎回费采用**统一费率**口径（不按持有天数分档）：`fundInfoProvider` 将 `redeemFeeTiers` 覆盖为单档 `[{ minHoldDays: 0, rate }]`，复用核心库既有 `calcRedeem`（费 = 赎回毛额 × 费率，净到手 = 毛额 − 费）无需改动结算逻辑。
- **变更（`@fund/core`）**：
  - `storage/repository.ts`：`AppSettings` 新增 `defaultRedeemFeeRate`（A，默认 0.005）与 `defaultRedeemFeeRateC`（C，默认 0.005）；`DEFAULT_SETTINGS` 补默认值（`SettingsRepository.get` 以 `{...DEFAULT_SETTINGS, ...stored}` 合并，旧设置自动补齐新字段，向后兼容）。
- **变更（`packages/web`）**：
  - `services/fundInfoService.ts`：新增 `redeemFeeRates` 状态与 `setRedeemFeeRates`、`redeemFeeFor(shareClass)`（C 用 C 费率、A/未知用 A 费率）；`fundInfoProvider` 在解析 `purchaseFeeRate` 的同时，按份额类别将 `redeemFeeTiers` 设为单档统一费率。
  - `stores/settingsStore.ts`：新增 `syncRedeemFeeRates` 并纳入 `syncDerived`（初始化与每次 `update` 时同步 A/C 赎回费率到 `fundInfoService`）。
  - `pages/SettingsPage.tsx`：在申购费两项之后新增「默认赎回费率（A 类基金）」与「赎回费率（C 类基金）」两项（与申购费同款 `InputNumber` + 百分比说明）。
  - 回测接入：`BacktestPage` 单策略回测传 `redeemFeeRate: settings.defaultRedeemFeeRate`；`ComparisonPanel` 新增 `redeemFeeRate` prop 并透传给各策略集回测（与 `purchaseFeeRate` 并列，由 `BacktestPage` 注入 `settings.defaultRedeemFeeRate`）。
- **测试**：web 新增 2 个用例（`fundInfoService.test.ts`：赎回费按份额类别解析为单档 `[{minHoldDays:0,rate}]`、设置变更即时生效），web 35→37。`pnpm -r typecheck`、`pnpm -r test`（core 218 + web 37 + workers 40 = **295 用例全绿**）、core+web 生产构建均通过；修改文件 eslint 0 错误。
- **端到端（本地 `pnpm dev` + 浏览器实测真实 161725）**：
  - **设置项**：设置页新增「默认赎回费率（A 类基金）0.50%」「赎回费率（C 类基金）0.50%」两项，默认值正确；将 A 改为 1% 后 `localStorage.fund.settings` 持久化 `defaultRedeemFeeRate=0.01 / defaultRedeemFeeRateC=0.005`。
  - **回测应用**：底仓¥100000 + 按份额卖出 2000 份回测 2024 上半年 → 逐笔卖出净额 = 份额 × 净值 × (1 − 1%)，实测 2024-02-07 毛额 1764.40→净额 1746.76、03-13 1929.20→1909.91、04-26 1915.20→1896.05，均精确等于 ×0.99，赎回费按配置 A 费率正确扣减。
  - 控制台仅既有 antd Form 开发期告警，实测后已清理 localStorage 测试数据与临时文件。
- **实现范围**：`@fund/core`（`AppSettings` 类型 + 默认值）与 `packages/web`（费率服务/设置 store/设置页/回测接入）；复用核心库 `calcRedeem` 与结算逻辑，未改动 `packages/workers` 与交易结算引擎。

> 累计测试：core 218 + web 37 + workers 40 = 295 用例全绿；core/web/workers 全量 typecheck 通过，core+web 生产构建通过。
